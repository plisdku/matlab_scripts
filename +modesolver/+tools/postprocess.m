...
% This block added by PCH
if numel(neff) > 1
    
    Hz = zeros(size(Hx,1), size(Hx,2), size(Hx,3));
    Ex = zeros(size(Hx,1)-1, size(Hx,2)-1, size(Hx,3));
    Ey = Ex;
    Ez = Ex;
    
    for nn = 1:length(neff)
        [Hz(:,:,nn), Ex(:,:,nn), Ey(:,:,nn), Ez(:,:,nn)] = ...
            modesolver.tools.postprocess(lambda, neff(nn), ...
            Hx(:,:,nn), Hy(:,:,nn), ...
            dx, dy, varargin{:});
    end
